...
const ImageGrayscaler = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/convert/image-grayscale', formData, {
        headers: {
      ...
      link.href = url;
      link.setAttribute('download', `grayscale-${selectedFile.name}`); // Set the download filename
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={onSubmit}>
        ...
        <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Convert to Grayscale</button>
      </form>
    </div>
  );
};
