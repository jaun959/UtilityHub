...
const PdfRotator = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/convert/pdf-rotate', formData, {
        headers: {
      ...
      link.href = url;
      link.setAttribute('download', `rotated-${Date.now()}.pdf`); // Set the download filename
